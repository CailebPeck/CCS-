# Coastline Hub

One mobile app for the Coastline group — built with [Capacitor](https://capacitorjs.com) so a single web codebase ships as a native iOS app (App Store) and Android app (Google Play).

**Inside the app:**

| Section | What it does |
| --- | --- |
| **Coastline Current Solutions** | Customer-facing pricing & booking app: browse the full product catalogue (~170 items with live sell pricing), build a job, request custom quotes, book a time slot. Staff mode adds a dashboard, quote builder with call-out fee + Xero/CRM push, an editable price list, and customer history. |
| **Coastline Street** | Streetwear label showcase — Collection 001 "Bloodline", Volt Division mascot series, mailing-list signup. |
| **CCS Tools** | Concept page for the Cable Retrieval & Inspection Tool with product roadmap. |

All state (cart, bookings, price overrides, staff mode) persists on-device via localStorage.

## Project layout

```
www/                  ← the app itself (plain HTML/CSS/JS — no build step)
  index.html
  css/app.css
  js/data.js          ← product catalogue, pricing (cost + markup), brand data
  js/store.js         ← state + persistence
  js/app.js           ← screens, navigation, event handling
  img/                ← brand marks, product shots, icons
  fonts/              ← Brothers, the brand typeface (see licence note below)
  manifest.json       ← PWA manifest, so the site installs to a home screen
brand-source/         ← the designer's logo masters (vector PDFs)
tools/generate-brand.py ← derives every brand mark, app icon and splash from them
tools/generate-art.js ← draws the category icons + tool illustration
android/              ← native Android project (generated by Capacitor, committed)
ios/                  ← native iOS project (generated by Capacitor, committed)
assets/               ← 1024px icon + 2732px splash source PNGs
resources/            ← editable SVG sources for icon/splash
capacitor.config.json ← app id: au.com.coastlinecurrentsolutions.hub
```

## Local development

```bash
npm install
npm start            # serves www/ at http://localhost:8080
```

Edit anything under `www/`, refresh the browser. When you're ready to test on-device:

```bash
npx cap sync         # copies www/ into android/ and ios/
```

## Website (GitHub Pages)

The same `www/` folder is published as a website, so customers can browse
services and request a booking in a browser with nothing to install:

**https://cailebpeck.github.io/CCS-/**

`.github/workflows/pages.yml` redeploys it on every push to `main` — the
website and the app are always the same code, so there is nothing to keep in
sync. On screens 640px and wider `css/app.css` swaps the phone tab bar for a
left sidebar and widens the grids; below that it renders exactly as the app does.

**One-time setup, required before the first deploy can succeed:**
Settings → Pages → Source → **GitHub Actions**, then re-run the workflow.

This cannot be automated. Creating the Pages site needs repo-admin rights, and
the workflow's `GITHUB_TOKEN` only carries `pages: write` — asking
`configure-pages` to do it fails with *"Resource not accessible by
integration"*. A repo admin has to flip it once, by hand.

Opening the site on a phone offers "Add to Home Screen" (via `manifest.json`),
which is the only way to get the app onto an iPhone before it is on TestFlight.

## Building for the stores

### Prerequisites

- **Android:** [Android Studio](https://developer.android.com/studio) (any OS). A Google Play developer account (US$25, one-off).
- **iOS:** A Mac with [Xcode 15+](https://developer.apple.com/xcode/), plus an [Apple Developer Program](https://developer.apple.com/programs/) membership (US$99/year).

### Android (Google Play) — cloud builds, no Android Studio needed

Every push to `main` (and every PR) triggers the **Android Build** GitHub Action
(`.github/workflows/android-build.yml`), which produces two downloadable artifacts
under the run's **Summary** page on the repo's **Actions** tab:

- `coastline-hub-release-aab` — the app bundle you upload to Google Play
- `coastline-hub-debug-apk` — installable directly on any Android phone for testing
  (enable "install from unknown sources", copy the APK over, tap it)

**One-time: create an upload keystore and add it as repo secrets.** Until these
secrets exist, the AAB builds unsigned and Play won't accept it.

No local tools needed — [Google Cloud Shell](https://shell.cloud.google.com) is a
free terminal in your browser with `keytool` preinstalled:

```bash
keytool -genkeypair -v -keystore upload-keystore.jks -alias upload \
  -keyalg RSA -keysize 2048 -validity 10000
base64 -w0 upload-keystore.jks > upload-keystore.b64
cloudshell download upload-keystore.jks     # back this up somewhere safe!
cloudshell download upload-keystore.b64
```

Then on GitHub → repo **Settings → Secrets and variables → Actions → New repository secret**, add:

| Secret | Value |
| --- | --- |
| `ANDROID_KEYSTORE_BASE64` | contents of `upload-keystore.b64` |
| `ANDROID_KEYSTORE_PASSWORD` | the keystore password you chose |
| `ANDROID_KEY_ALIAS` | `upload` |
| `ANDROID_KEY_PASSWORD` | the key password (same as keystore password unless you set one) |

**Keep `upload-keystore.jks` backed up privately** (password manager, offline drive).
Never commit it. If it's lost, Play can reset your upload key, but it's a support process.

**Then, per release:**

1. Push to `main` (or press **Run workflow** on the Actions tab), wait for the green tick, download `coastline-hub-release-aab`.
2. In [Google Play Console](https://play.google.com/console): create the app (name **Coastline Hub**, package `au.com.coastlinecurrentsolutions.hub`), enroll in **Play App Signing** (default), upload the AAB under **Production → Create release**.
3. Complete the store listing (copy in `STORE_LISTING.md`), content rating questionnaire, data-safety form (see `PRIVACY_POLICY.md` — the app collects no data), and privacy policy URL (host `PRIVACY_POLICY.md` on your website, e.g. `coastlinecurrentsolutions.com.au/app-privacy`).
4. Submit for review. First review typically takes a few days.

Local command-line alternative (needs Android SDK + JDK 21):

```bash
cd android && ./gradlew bundleRelease
# output: android/app/build/outputs/bundle/release/app-release.aab
```

### iOS (App Store) — cloud builds, no Mac needed

The **iOS Build & TestFlight Upload** workflow (`.github/workflows/ios-build.yml`)
builds and signs the app on a GitHub-hosted Mac and uploads it straight to
App Store Connect. You trigger it from the **Actions** tab — no Mac, no Xcode.
Every step below works from a browser (iPad included).

**One-time setup:**

1. **Join the Apple Developer Program** (US$99/yr) — enrol at
   [developer.apple.com/programs/enroll](https://developer.apple.com/programs/enroll)
   or via the **Apple Developer** app on iPhone/iPad. Approval can take a day or two.
2. **Register the app's bundle ID** — [developer.apple.com/account](https://developer.apple.com/account)
   → Certificates, Identifiers & Profiles → **Identifiers** → **+** → App IDs → App →
   Bundle ID (explicit): `au.com.coastlinecurrentsolutions.hub`, description "Coastline Hub".
3. **Note your Team ID** — shown under **Membership details** on the same site (10 characters, e.g. `A1BC23DEF4`).
4. **Create the app record** — [appstoreconnect.apple.com](https://appstoreconnect.apple.com)
   → Apps → **+** → New App: platform iOS, name **Coastline Hub**, the bundle ID from step 2, any SKU (e.g. `coastline-hub`).
5. **Create an API key** — App Store Connect → **Users and Access → Integrations → Team Keys**
   → Generate API Key, role **App Manager**. Note the **Key ID** and **Issuer ID**, and
   download the `.p8` file (**one chance only** — keep it safe).
6. **Base64 the key** — in [Google Cloud Shell](https://shell.cloud.google.com)
   (upload the `.p8` via the ⋮ menu): `base64 -w0 AuthKey_XXXXXXXXXX.p8` and copy the output.
7. **Add four repo secrets** — GitHub → repo **Settings → Secrets and variables → Actions**:

   | Secret | Value |
   | --- | --- |
   | `APPLE_TEAM_ID` | your Team ID (step 3) |
   | `APP_STORE_CONNECT_KEY_ID` | the Key ID (step 5) |
   | `APP_STORE_CONNECT_ISSUER_ID` | the Issuer ID (step 5) |
   | `APP_STORE_CONNECT_KEY_B64` | the base64 string (step 6) |

**Then, per release:**

1. **Actions** tab → **iOS Build & TestFlight Upload** → **Run workflow**. Signing
   certificates and provisioning profiles are created and managed automatically in
   Apple's cloud — nothing to install anywhere.
2. ~15 minutes later the build appears in App Store Connect → **TestFlight**.
   Install it on your own iPhone/iPad via the TestFlight app to try it.
3. When happy: in App Store Connect, fill in the listing (copy in `STORE_LISTING.md`),
   screenshots (6.7" and 6.5" iPhone sizes), privacy ("Data not collected"), privacy
   policy URL, attach the build to the version, and **Submit for Review**.

**Alternative:** [Codemagic](https://codemagic.io) (free tier) connects to this repo and
does the same job with a guided UI — hand it the same App Store Connect API key and it
manages signing and TestFlight uploads for you.

### Updating the app later

1. Edit `www/`, bump `version` in `package.json`, and:
   - Android: bump `versionCode`/`versionName` in `android/app/build.gradle`
   - iOS: bump version/build in Xcode project settings
2. `npx cap sync`
3. Rebuild + re-upload through the same flow.

### Artwork and branding

Three kinds of image live in the repo:

**Brand marks** (`www/img/brand/`) — derived from the designer's real logo
files in `brand-source/` by `tools/generate-brand.py`:

- `roundel.webp` — the wave + bolt mark, used in the hub and every header
- `wordmark.webp` / `wordmark-dark.webp` — the COASTLINE / CURRENT SOLUTIONS
  lockup, light and dark colourways
- `badge.webp` — the full circular badge with ELECTRICAL around the rim
- `icon-*.png` — PWA icons referenced by `www/manifest.json`

The same script writes `assets/icon.png` and the splash screens, so the store
artwork and the in-app marks can never drift apart.

```bash
python3 tools/generate-brand.py    # needs poppler-utils + pillow
npx cap sync
```

These are rasters rather than SVG on purpose. The masters convert to SVG
cleanly, but the wave is so path-heavy that each mark lands around 190 KB —
and the app only ever shows the roundel at 96px and 34px, where a 512px WebP
is both far smaller and pixel-identical. `brand-source/*.pdf` stays in the repo
as the scalable source of truth, so any size can be re-rendered on demand.

**Product photography** (`www/img/street/*.webp`) — cropped from the Coastline
brand sheets and product mockups. To swap in a better shot, replace the file
keeping the same name; the id in `STREET_PRODUCTS` / `STREET_VOLT`
(`www/js/data.js`) is the filename stem.

**Icons and illustrations** — drawn in code by `tools/generate-art.js`, since
they have no vector master:

- `cat/*.svg` — the 13 electrical category icons
- `tools/tool.svg` — the CCS Tools hook + camera probe illustration

```bash
node tools/generate-art.js && npx cap sync
```

### Typeface

Headings are set in **Brothers**, the face used in the logo lockups,
self-hosted at `www/fonts/brothers.woff2` so it renders identically in the app,
on the website and offline.

> **Check the licence before this goes public.** Brothers is a commercial
> typeface — `Copyright (C) 1999, Emigre, Inc. Designed by John Downer`.
> Self-hosting it on a website is a separate licence from desktop or logo use.
> If a webfont licence isn't in place, either buy one from Emigre or swap the
> `@font-face` in `www/css/app.css` for a freely licensed face; the font stack
> already falls back cleanly.

### Regenerating icons/splash

```bash
python3 tools/generate-brand.py   # rebuilds assets/icon.png + splash from the masters
npm run assets                    # fans those out to every iOS/Android size
npx cap sync
```

> `npm run assets` passes `--ios --android` deliberately. Without those flags
> `@capacitor/assets` also generates a PWA set into a root `icons/` folder and
> **rewrites `www/manifest.json`** to point at `../icons/*.webp` — paths that do
> not resolve from inside `www/`, which silently breaks "Add to Home Screen".
> The manifest is maintained by hand and its icons come from
> `tools/generate-brand.py`; don't drop the flags.


Edit `resources/icon.svg` / `resources/splash.svg`, export to `assets/icon.png` (1024×1024) and `assets/splash.png` (2732×2732), then:

```bash
npm run assets && npx cap sync
```

## Store-readiness notes

- **Guideline 4.2 (minimum functionality), Apple:** Apple rejects apps that are "just a website". This app runs fully offline, has interactive tooling (job builder, quote calculator, editable price list) and on-device persistence, which is the right side of that line. Keep adding native value (push notifications for booking updates via `@capacitor/push-notifications` is the natural next step).
- **Bookings are device-local today.** The booking flow stores jobs on the device and shows a confirmation; there is no backend yet. Before taking real bookings, wire `Store.submitBooking()` (in `www/js/store.js`) to your backend/email/SMS service so requests actually reach the office. Until then, treat store builds as a catalogue + quoting tool.
- **Xero / CRM buttons** in staff mode are visual placeholders — wire them to real integrations when ready.
- **Product images** hot-link Sparky Direct product pages and fall back to a category tile when offline or if an image disappears.
